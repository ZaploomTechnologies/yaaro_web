  ...
  return (
    <div className="flex items-center justify-center gap-3 flex-wrap">
      ...
      <motion.a
        ...
        whileTap={{ scale: 0.96 }}
        className="flex items-center gap-2.5 bg-[#14140F] rounded-xl px-5 py-3 hover:brightness-110 transition-all"
      >
...
export default function ReferralPage({ referralCode: rawCode, initialData }) {
    ...
    return (
      <div className="min-h-screen bg-[#F7F6F2] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[#6E6A5D] text-sm animate-pulse">Loading referral…</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#F7F6F2] flex flex-col items-center justify-center p-6 text-center">
        ...
        >
          ...
          <h1 className="text-2xl font-bold text-[#14140F] mb-2">Invalid Referral Code</h1>
          <p className="text-[#6E6A5D] mb-8 max-w-xs mx-auto">
            The referral code <strong className="text-[#14140F]">{rawCode || '(none)'}</strong> is not
            a valid Yaaro referral code. Please check the link and try again.
          </p>
          <Link
            href="/"
            className="inline-block bg-primary text-[#14140F] font-bold px-8 py-3 rounded-full hover:scale-105 transition-transform"
          >
            Back to Home
          </Link>
        </motion.div>
    ...
    return (
      <div className="min-h-screen bg-[#F7F6F2] flex flex-col items-center justify-center p-6 text-center">
        ...
        >
          ...
          <h1 className="text-2xl font-bold text-[#14140F] mb-2">Code Not Found</h1>
          <p className="text-[#6E6A5D] mb-8 max-w-xs mx-auto">
            Referral code <strong className="text-[#14140F]">{rawCode}</strong> doesn&apos;t exist or
  ...
  return (
    <div className="relative min-h-screen bg-[#F7F6F2] flex flex-col overflow-hidden">
      ...
      {/* ── Header ── */}
      <header className="relative w-full px-6 py-4 flex items-center justify-between border-b border-[#14140F]/10">
        ...
        <motion.a
          href="#"
          whileTap={{ scale: 0.96 }}
          className="text-sm font-semibold bg-primary text-[#14140F] px-4 py-2 rounded-full hover:opacity-90 transition-opacity"
        >
          Download App
        </motion.a>
      </header>
      ...
      <main className="relative flex-1 flex flex-col items-center px-4 py-12">
        ...
        >
          {/* Profile photo */}
          <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-primary/40 shrink-0">
          ...
          <div className="space-y-1">
            <p className="text-xl font-bold text-[#14140F]">{referrer.fullName}</p>
            <p className="text-[#6E6A5D] text-base">invites you to join Yaaro</p>
          </div>
          ...
          {/* Divider */}
          <div className="w-full h-px bg-[#14140F]/10" />

          {/* How to join */}
          <div className="w-full text-left space-y-4">
            <h2 className="text-center text-lg font-bold text-[#14140F] tracking-wide">
              How to join Yaaro
            </h2>
            ...
            <ol className="space-y-3">
              {steps.map((step) => (
                <li key={step.n} className="flex items-start gap-3">
                  <span className="mt-0.5 w-6 h-6 rounded-full bg-primary/20 border border-primary/30 flex items-center justify-center shrink-0 text-xs font-bold text-[#14140F]">
                    {step.n}
                  </span>
                  <span className="text-[#6E6A5D] text-sm leading-relaxed">
                    {step.text}
                  </span>
                </li>
              ))}
            ...
            <div className="bg-white/80 border border-primary/25 shadow-[0_20px_40px_-30px_rgba(20,20,15,0.25)] rounded-2xl px-6 py-5 text-center space-y-1">
              <p className="text-xs text-[#8A8574] uppercase tracking-widest">
                Your referral code
              </p>
              <p className="text-4xl font-extrabold text-[#14140F] tracking-widest">
